...
*   **Supported Hardware**:
      ...
      *   **ESP32-C3**: **NOT RECOMMENDED**. The C3 is single-core, and since the effects are computationally intensive, running them alongside Wi-Fi on a single core can cause stuttering and stability issues. V1.41 treats C3 RMT as experimental after physical tests showed artifacts with the default RMT path. C3 supports up to 2 experimental RMT strips or 1 experimental SPI strip; 2000 virtual SPI LEDs held ~58-60 `LedFPS`, but long physical SPI strips are still pending validation.
