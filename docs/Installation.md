...
*   **Supported Hardware**:
      ...
      *   **ESP32-C3**: **NOT RECOMMENDED**. The C3 is single-core, and since the effects are computationally intensive, running them alongside Wi-Fi on a single core can cause stuttering and stability issues. V1.41 treats C3 RMT as experimental: one RMT output reached ~54-56 `LedFPS` at 360 LEDs and ~35 `LedFPS` at 600 LEDs, but several demanding effects still showed sparse pixel artifacts. C3 supports up to 2 experimental RMT strips or 1 experimental SPI strip; 2000 virtual SPI LEDs held ~58-60 `LedFPS`, but long physical SPI strips are still pending validation.
