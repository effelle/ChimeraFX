...
*   **Anti-Flicker & Debounce Protection:** 
    *   The dimmer uses ESPHome's native smooth brightness ramp for ordinary lights. On release, ChimeraFX settles to the commanded stop point with a very short transition so the ramp does not snap. When an active effect needs direct brightness updates, ChimeraFX still computes the release point from the commanded ramp timeline instead of sampling ESPHome's internal transition state.
