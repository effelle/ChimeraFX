...
# Find indices using regex for flexibility
idx_172 = -1
idx_177 = -1

for i, line in enumerate(lines):
    if re.search(r'172 \| Sonar Reveal', line):
        idx_172 = i
    if re.search(r'177 \| Resonance Fill', line):
        idx_177 = i

if idx_172 != -1 and idx_177 != -1:
    new_lines = lines[:idx_172] + sonar_block + ['\n'] + venetian_block + ['\n'] + lines[idx_177:]
    with open(path, 'w', encoding='utf-8') as f:
        f.writelines(new_lines)
    print("Documentation cleaned successfully (Stage 3).")
else:
    print(f"Error: Indices not found. 172: {idx_172}, 177: {idx_177}")
