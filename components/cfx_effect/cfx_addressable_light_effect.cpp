...
void CFXAddressableLightEffect::stop() {
  ...
  void CFXAddressableLightEffect::apply(light::AddressableLight & it,
    ...
    if (this->intro_active_) {
      ...
      if (dur_num != nullptr && dur_num->has_state()) {
        ...
        duration_ms =
            (uint32_t)(this->intro_duration_preset_.value() * 1000.0f);
      } else {
  ...
  uint8_t CFXAddressableLightEffect::get_default_palette_id_(
      uint8_t effect_id) {
  ...
  void CFXAddressableLightEffect::run_controls_() {
    ...
    // Handle expected ON state, but detecting manual UI overrides
      ...
      if (c && c->get_target_segment() &&
          c->get_target_segment()->has_state()) {
  ...
  void CFXAddressableLightEffect::run_intro(light::AddressableLight & it,
    ...
    c = Color(
        (uint8_t)(c.r * user_brightness), (uint8_t)(c.g * user_brightness),
      ...
      for (int i = 0; i < logical_len; i++) {
        ...
        if (alpha > 0.0f) {
          if (use_palette && ::instance) {
            ...
            uint32_t cp = ::instance->_segment.color_from_palette(
          ...
          if (alpha < 1.0f) {
            pixel_c = Color(
                (uint8_t)(pixel_c.r * alpha), (uint8_t)(pixel_c.g * alpha),
    ...
    case INTRO_MODE_GLITTER: {
      ...
      for (int i = 0; i < seg_len; i++) {
        ...
        if (active) {
          if (use_palette && ::instance) {
            uint8_t map_idx =
                (uint8_t)((i * 255) / (seg_len > 0 ? seg_len : 1));
            uint32_t cp = ::instance->_segment.color_from_palette(
                map_idx, false, true, 255, 255);
            pixel_c = Color((uint8_t)(((cp >> 16) & 0xFF) * user_brightness),
                            (uint8_t)(((cp >> 8) & 0xFF) * user_brightness),
    ...
    case INTRO_MODE_TWIN_PULSE: {
      ...
      if (c_size < 3.0f)
        c_size = 3.0f;
      float short_gap =
          length * 0.12f; // Gap between cursors (increased to 12%)
      if (short_gap < 1.0f)
        short_gap = 1.0f;
