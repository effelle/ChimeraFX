...
uint16_t mode_fire_2012(void) {
  ...
  for (int j = 0; j < len; j++) {
    ...
    if (t <= 85) {
      ...
      g = 255;
      b = (t - 170) * 3;
      uint8_t w = 0; // Fire inherently has no white, but if a palette had it...
                     // actually Fire is hardcoded RGB here.
    ...
    uint16_t mode_fire_dual(void) {
      ...
      for (int i = 0; i < VIRTUAL_HEIGHT; i++) {
        uint8_t cool = (it != instance->_segment.step)
                           ? random8((((20 + timing.wled_speed / 3) * 16) /
                                      VIRTUAL_HEIGHT) +
                                     2)
                           : random8(4);
      ...
      // Remove vacuum: The flames should meet exactly in the middle without a
      // black gap. For odd lengths, the exact center pixel is written by both
      // halves (perfect overlap).
      int half_len = (len + 1) / 2;
      if (half_len < 1)
        half_len = 1; // Safety for very short strips

      // Scale factor: Virtual -> Physical Half
      // "Zoom": Map only the bottom 48 pixels of the 60px virtual fire to the
      // strip This crops the top ~20% (mostly smoke/black).
      float scale = (float)(VIRTUAL_HEIGHT - 12) / (float)half_len;

      // Helper lambda for heat-to-color conversion
      auto heat_to_rgb = [&heat](int v_index, uint8_t &r, uint8_t &g,
                                 uint8_t &b) {
        uint8_t t = min(heat[v_index], (uint8_t)240);
        if (t <= 85) {
          r = t * 3;
      ...
      if (mirror_mode) {
        ...
        // Center of strip has the hottest fire (low heat indices), edges have
        // the coolest (high heat indices)

        // Left half: Center -> Left edge
        // j=0 should place HOTTEST (v_index near 0) at CENTER, coolest at edge
        for (int j = 0; j < half_len; j++) {
          // Reverse the v_index: j=0 gets low heat index (hot), j=half_len-1
          // gets high (cool)
          ...
          uint8_t w = 0; // Fire is an RGB effect natively.
          instance->_segment.setPixelColor(j, RGBW32(r, g, b, w));
        }
        ...
        for (int j = 0; j < half_len; j++) {
          ...
          uint8_t w = 0; // Fire is an RGB effect natively.
          instance->_segment.setPixelColor(len - 1 - j, RGBW32(r, g, b, w));
        }
      } else {
    ...
    static void pacifica_init_caches() {
      ...
      // pacifica_palette_1)
      static const CRGBPalette16 pal1 = {
      ...
      // pacifica_palette_2)
      static const CRGBPalette16 pal2 = {
          0x000507, 0x000409, 0x00030B, 0x00030D, 0x000210, 0x000212,
          0x000114, 0x000117, 0x000019, 0x00001C, 0x000026, 0x000031,
      ...
      // and 4
      static const CRGBPalette16 pal3 = {
    ...
    static void pacifica_one_layer_wled(CRGB & c, uint16_t i, uint8_t cache_id,
      ...
      // WLED EXACT: unsigned cs = scale16(s16, wavescale_half) +
      // wavescale_half;
    ...
    static void pacifica_deepen_colors(CRGB & c) {
      ...
      // blue tint. WLED uses (2, 5, 7) but ESPHome needs much higher for
      // visibility after gamma.
    ...
    // intensity
    static void pacifica_one_layer_zoomed(
    ...
    // --- Ocean Effect ---
    // Inspired by WLED's Pacifica, optimized for long strips and ambient
    // lighting. Uses bidirectional wave interference with collision-based
    // whitecaps.
    uint16_t mode_ocean() {
      // === FRAME DIAGNOSTICS (enabled with CFX_FRAME_DIAGNOSTICS) ===
      ...
      for (int i = 0; i < len; i++) {
        ...
        // Color deepening (teal character) - REDUCED scaling to preserve
        // brightness
        ...
        // GAMMA CORRECTION: Apply gamma to final linear output to simulate
        // "deep ocean" contrast which was previously provided by the driver's
        // gamma correction.
        instance->_segment.setPixelColor(
            i, RGBW32(instance->applyGamma(c.r), instance->applyGamma(c.g),
    ...
    uint16_t mode_plasma(void) {
      ...
      uint8_t phase2_raw =
          (slowTime * (7 + aux_offset)) >> 2; // Slightly different
      ...
      // spatialScale=5 This prevents the "noisy" 5-6 LED sequences at high
      // speed values
      uint8_t speed = instance->_segment.speed;
      ...
      for (int i = 0; i < len; i++) {
        ...
        // Shifted so intensity=128 (default) gives same fill as intensity=90
        // would This provides more voids at the default setting
        int16_t shifted = (int16_t)intensity - 38; // Shift down by 38
        if (shifted < 0)
          shifted = 0;
        uint8_t fillAmount =
            ((uint16_t)shifted * shifted) >> 8; // Quadratic: 0-185
        ...
        uint16_t brightness16 =
            gammaBri + ((fillAmount * (255 - gammaBri)) >> 8);
    ...
    uint16_t mode_breath(void) {
      ...
      for (int i = 0; i < len; i++) {
        ...
        // This ensures that even with palettes, the "off" state matches the
        // "on" color.
        bgR = (fgR * 54) >> 8;
        bgG = (fgG * 54) >> 8;
        bgB = (fgB * 54) >> 8;
...
#define SHADOW_CLR(idx) (shadow[(idx) >> 3] &= ~(1 << ((idx) & 7)))
      ...
      // Hold time: Speed 255 = 500ms (fast cycle), Speed 0 = 3000ms (slow
      // cycle)
      ...
      case 0: { // FILLING - set random OFF pixels to ON using probability
        ...
        for (int n = 0; n < pixels_to_spawn && pixel_count < fill_threshold;
             n++) {
      ...
      for (int i = 0; i < len; i++) {
        if (SHADOW_GET(i)) {
          if (use_rainbow) {
            ...
            col.w = 0;
            instance->_segment.setPixelColor(
                i, RGBW32(col.r, col.g, col.b, col.w));
          } else {
...
#undef SHADOW_CLR
    ...
    uint16_t mode_flow(void) {
      ...
      // WLED: counter = strip.now * ((SEGMENT.speed >> 2) +1); counter =
      // counter >> 8;
      unsigned counter = 0;
      ...
      for (int z = 0; z < zones; z++) {
        ...
        for (int i = 0; i < zoneLen; i++) {
          ...
          CRGBW c = ColorFromPalette(active_palette, colorIndex, 255);
          instance->_segment.setPixelColor(pos + led,
    ...
    uint16_t mode_phased(void) {
      ...
      // Intensity controls the spatial frequency (how tightly packed the nodes
      // are) Maps 0-255 to 1..10 sine waves across the total strip length
      uint32_t num_waves = 1 + (intensity / 28);

      // Calculate 16-bit phase delta per pixel so the math is continuous across
      // any length
      ...
      for (int i = 0; i < len; i++) {
        ...
        // Standard Phased color mapping (spread the palette seamlessly across
        // the strip)
    ...
    uint16_t mode_ripple(void) {
      ...
      if (random16(65535) <=
          (uint32_t)(instance->_segment.intensity * delta * 3)) {
      ...
      for (int i = 0; i < maxRipples; i++) {
        ...
        // resolution of 'age' instead of the quantized 'ripplestate'. WLED:
        // prop = (state / decay) * (speed + 1). (Approx) Ours: prop = (age *
        // (speed + 1)) / decay / 256. This gives us the same scale as WLED but
        // updates every frame.
        ...
        // This was caused by Step 48 wrapping (6 * 48 = 288 > 256). It created
        // a trough.
        //
        ...
        for (int v = 0; v < 6; v++) {
          ...
          if (mag > 0) {
            ...
            auto apply_max_pixel = [&](int pos, uint32_t color,
                                       uint8_t magnitude) {
              CRGBW c_new(color);
              ...
              // This prevents the "tail" of a wave from darkening the "trail"
              // of another.
    ...
    uint16_t mode_meteor(void) {
      ...
      // 128+random(127)) } High intensity = fewer pixels decay = longer trail
      // Low intensity = more pixels decay = shorter trail
      for (int i = 0; i < len; i++) {
      ...
      // Floor-sweeper: removes any sub-threshold residue that scale8 can't
      // clear. Value of 1 is minimal — won't affect the visible trail, only
      // kills pixels stuck at 1-2 brightness from asymptotic multiplicative
      // fade.
    ...
    static uint16_t chase(uint32_t color1, uint32_t color2, uint32_t color3,
      ...
      // Based on intensity: Low intensity = 1 pixel. High intensity = full
      // strip length
      float size =
          1.0f + ((float)(instance->_segment.intensity * len) / 1024.0f);
      ...
      for (unsigned i = 0; i < len; i++) {
        ...
        if (local_phase < size) {
          ...
          if (dist_edge < fade_zone) {
            ...
            final_color =
                color_blend(final_color, color2, (uint8_t)(mix * 255.0f));
          } else {
            final_color = color2;
          }
          ...
          if (dist_edge < fade_zone) {
            // Blend edge with Zone 1 (color2) or background depending on which
            // side
            float mix = 0.5f + (dist_edge / (2.0f * fade_zone));

            uint32_t mix_target = dist_start < dist_end ? color2 : final_color;
            final_color =
                color_blend(mix_target, color3, (uint8_t)(mix * 255.0f));
          } else {
            final_color = color3;
          }
        }
    ...
    uint16_t mode_chase_color(void) {
      bool do_palette = (instance->_segment.palette != 255 &&
                         instance->_segment.palette != 0);
    ...
    uint16_t mode_bpm(void) {
      ...
      // Rather than a soft sine wave, we want a punchy "surge" that feels
      // musical. We sharpen the sine wave into a spiky pulse by squaring it.
      ...
      for (int i = 0; i < len; i++) {
        ...
        // MULTIPLY the wave by the master beat envelope. This completely
        // prevents 8-bit wrap tearing. Base brightness is 40 (never fully dark)
        // + (wave * surge pulse)
    ...
    uint16_t mode_glitter(void) {
      ...
      // Directions: "Fill the entire strip with a rainbow gradient...
      // Subtracting time moves it backwards" Use active palette (defaulting to
      // Rainbow if none selected or if default/0 is selected)
      const uint32_t *active_palette =
          (instance->_segment.palette == 0)
    ...
    uint16_t mode_chase_multi(void) {
      ...
      float width =
          1.0f + (instance->_segment.intensity / 16.0f); // 1.0 to 16.9
      ...
      for (unsigned i = 0; i < len; i++) {
        ...
        if (local_phase < width) {
          ...
          dist_to_edge =
              bg_start_dist < bg_end_dist ? bg_start_dist : bg_end_dist;
        ...
        if (instance->_segment.palette == 255 ||
            instance->_segment.palette == 0) {
          bg_color =
              instance->_segment.colors[1]; // Use secondary (often black)
        } else {
        ...
        if (mix >= 1.0f) {
          ...
          final_color =
              color_blend(bg_color, fg_color, (uint8_t)(mix * 255.0f));
        }
    ...
    uint16_t mode_sunrise(void) {
      ...
      for (int i = 0; i <= len / 2; i++) {
        ...
        instance->_segment.setPixelColor(i, RGBW32(c.r, c.g, c.b, c.w));
        instance->_segment.setPixelColor(len - i - 1,
                                         RGBW32(c.r, c.g, c.b, c.w));
      }
    ...
    uint16_t mode_sparkle(void) {
      ...
      // final_fade can be so small that pixels accumulate faster than they
      // drain. Enforce a minimum fade that guarantees visible drain at all
      // speeds.
      uint8_t min_fade;
      if (instance->_segment.speed == 0)
      ...
      if (cfx::hw_random16(0, 255) < chance) {
        ...
        if (instance->_segment.palette != 0 &&
          ...
          color = instance->_segment.color_from_palette(colorIndex, true, false,
                                                        0, 255);
        }
    ...
    uint16_t mode_hyper_sparkle(void) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        if (instance->_segment.palette != 0 &&
            instance->_segment.palette != 255) {
          color =
              instance->_segment.color_from_palette(index, true, false, 0, 255);
        }
        instance->_segment.setPixelColor(index, color);
      }
...
#define MAX_ENERGY_SPARKS 10
    ...
    // Progress bar that unmasks a live rainbow animation with a white leading
    // tip. Phase 1: Agitation Engine - Noise-driven speed fluctuations. Phase
    // 2: Energy Spikes - Localized white-hot eruptions during high chaos. Phase
    // 3: Contrast & Size Refinement - Hue-gating and 5-LED bloom. Phase 4:
    // Scaling & Exit Refinement - Proportional blooms and linear exit.
    ...
    uint16_t mode_chaos_theory(void) {
      ...
      if (!data->intro_done) {
        ...
        if (elapsed > intro_duration_ms) {
          ...
          uint8_t spawn_chance =
              cfx::scale8(180, progress); // High initial density

          if (cfx::hw_random8() < spawn_chance) {
            uint8_t sparks_to_spawn =
                (cfx::hw_random8() % max_sparks_frame) + 1;
      ...
      // global beat strikes. The higher the beat peak, the higher the
      // probability of spawning a spark.
        ...
        if (data->sparks[s].level > 0) {
          ...
          auto add_brightness = [&](int pos, uint8_t amount) {
            if (pos >= 0 && pos < len) {
              ...
              // Blend from the existing color towards pure RGB white (no W
              // channel to save power), keeping the background visible through
              // the spike edge
              uint32_t final =
                  color_blend(existing, (uint32_t)0x00FFFFFF, amount);
    ...
    uint16_t mode_scanner_internal(bool dualMode) {
      ...
      drawTrail(instance->_segment.aux1, instance->_segment.aux0, trail_len,
                255);
    ...
    // Replicates WLED logic with framerate-independent decay and gamma
    // correction
    uint16_t mode_heartbeat(void) {
      ...
      instance->_segment.aux1 =
          (uint16_t)((float)instance->_segment.aux1 * decay);
    ...
    struct Spark {
      ...
      Spark()
          : pos(0.0f), vel(0.0f), col(0), colIndex(0) {
    ...
    uint16_t mode_exploding_fireworks(void) {
      ...
      // clearing) and blur (trail smoothing). 10 is the subtractive amount
      // (approx 4% per frame).
      ...
      if (instance->_segment.aux0 < 2) {    // STATE: FLARE LAUNCH
        ...
        if (instance->_segment.aux0 == 2) {
          ...
          for (int i = 1; i < nSparks; i++) {
            ...
            sparks[i].vel =
                (float(cfx::hw_random16(0, 20001)) / 10000.0f) - 0.9f;
            sparks[i].vel *=
                -gravity * 50.0f *
          ...
          for (int i = 1; i < numSparks; i++) {
            if (sparks[i].col > 0) {
              ...
              if (sparks[i].pos >= 0 && sparks[i].pos < len) {
                ...
                instance->_segment.setPixelColor(
                    (int)sparks[i].pos, RGBW32(finalColor.r, finalColor.g,
                                               finalColor.b, finalColor.w));
              }
            }
          }
          *dying_gravity *=
    ...
    uint16_t mode_popcorn(void) {
      ...
      // < 255)`. Wait, WLED `mode_popcorn` spawns based on simple chance?
      // Actually, WLED default particle count logic might be sparser. We'll
      // scale density down. If user says "83 to have 1:1", it means 128
      // (default) is 1.5x too dense. Let's scale intensity by 0.65?
      ...
      for (int i = 0; i < numPopcorn; i++) {
        if (popcorn[i].pos >= 0.0f) { // Active
          ...
          if (cfx::hw_random8() < 5) { // Pop Chance
            ...
            if (instance->_segment.palette == 0) {
              popcorn[i].colIndex =
                  cfx::hw_random8(0, 3); // Pick simple colors?
            } else {
    ...
    uint16_t mode_dropping_time(void) {
      ...
      // User said: "Every drop reduced by one the strip length... At 100% all
      // lit and animation stop." This implies the level is the visual
      ...
      uint32_t estFallMs =
          (uint32_t)(estFallFrames * 15.0f); // Approx 15ms/frame
      ...
      if (state->fillingDropActive) {
        ...
        if (elapsed > nextPixelTime + 1500 &&
            targetLevel > state->filledPixels) {
      ...
      // If the palette center happens to be black, fallback to a light water
      // blue
      if (dropColor == 0)
        dropColor = 0x80C0FF;
      ...
      if (state->fillingDropActive) {
        if (state->fillingDrop.colIndex == 1) { // Forming
          ...
          if (pos >= state->filledPixels && pos < len) {
            instance->_segment.setPixelColor(pos,
                                             color_blend(dropColor, 0, 150));
          }
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (state->dummyDrops[i].colIndex != 0) {
          if (state->dummyDrops[i].colIndex == 1) { // Forming
            instance->_segment.setPixelColor(
                len - 1,
                color_blend(dropColor, 0, 255 - state->dummyDrops[i].col));
          } else {
            ...
            if (pos >= state->filledPixels && pos < len) {
              ...
              if (state->dummyDrops[i].colIndex ==
                  2) { // Tail (Only when falling)
                for (int t = 1; t <= 3; t++) {
                  int tPos = pos + t;
                  if (tPos >= state->filledPixels && tPos < len) {
                ...
                if (state->filledPixels < len && state->filledPixels >= 0) {
                  uint32_t cur =
                      instance->_segment.getPixelColor(state->filledPixels);
                  instance->_segment.setPixelColor(
                      state->filledPixels, color_blend(cur, head_col, 150));
                }
              }
            }
          }
      ...
      for (int j = 0; j < numDrops; j++) {
        ...
        if (drops[j].colIndex > 1) { // Falling
          if (drops[j].pos > 0) {
            ...
            // (colIndex == 2) User: "another led bounce 6 led backward with a
            // lower brightness without tail" So ONLY draw tail if falling.
            if (drops[j].colIndex == 2 && drops[j].vel < 0) {
              ...
              for (int t = 1; t <= 6; t++) {
                ...
                if (tPos >= 0 && tPos < len) {
                  ...
                  instance->_segment.setPixelColor(
                      tPos, color_blend(col, 0, 255 - dim));
                }
              }
            }
            ...
            if (drops[j].colIndex > 2) { // Bouncing
              ...
              // above if pos > 0) But we want it to be dimmer too. The main
              // logic above draws 'col' at 'pos'. We need to overwrite it with
              // dimCol if we are bouncing.
              if (pos >= 0 && pos < len) {
            ...
            if (drops[j].colIndex >
                2) {                 // Already bouncing and hit bottom again
...
#define MAX_BALLS 8
    ...
    uint16_t mode_bouncing_balls(void) {
      ...
      // fadeToBlackBy(160): aggressive multiplicative fade (37%
      // retention/frame). Clears a 255 pixel in ~4 frames = virtually no tail.
      // No grey floor risk at this fade strength, so subtractive_fade_val not
      // needed here.
      ...
      // Controls
      uint8_t numBalls =
          (instance->_segment.intensity * (MAX_BALLS - 1)) / 255 + 1;
    ...
    uint16_t blink(uint32_t color1, uint32_t color2, bool strobe,
      ...
      if (color == color1 && do_palette && instance->_segment.palette != 0 &&
        ...
        for (unsigned i = 0; i < instance->_segment.length(); i++) {
          // We use colors[0] vs colors[1] logic above but if do_palette is
          // true, we ignore color1 and use the palette color. WLED logic:
          // SEGMENT.color_from_palette(i, true, PALETTE_SOLID_WRAP, 0) Since we
          // lack simple palette helper in this scope, we use manual:
          const uint32_t *active_palette =
              getPaletteByIndex(instance->_segment.palette);
    ...
    uint16_t mode_multi_strobe(void) {
      ...
      bool isOn = ((instance->_segment.aux1 & 1) == 0) &&
                  (instance->_segment.aux1 < count);
        ...
        if (dual) {
          ...
          if (instance->_segment.palette == 0 ||
            ...
            const uint32_t *active_palette =
                getPaletteByIndex(instance->_segment.palette);
            CRGBW c =
                ColorFromPalette(active_palette, (i * 255) / len + 128, 255);
    ...
    uint16_t color_wipe(bool rev, bool useRandomColors) {
      ...
      if (!useRandomColors && (instance->_segment.palette == 0 ||
                               instance->_segment.palette == 255)) {
        fillSolidPalette(instance->_segment.colors[0]);
      }
      ...
      if (useRandomColors) {
        CRGBW c1 =
            ColorFromPalette(active_palette, instance->_segment.aux1, 255);
      ...
      for (int i = 0; i < len; i++) {
        ...
        if (useRandomColors) {
          CRGBW c0 =
              ColorFromPalette(active_palette, instance->_segment.aux0, 255);
    ...
    uint16_t mode_heartbeat_center(void) {
      ...
      // Dynamic Radius
      // Scale max_radius HIGHER than the strip length to ensure the fade
      // doesn't cutoff the edges at full brightness. Using 'len' instead of
      // 'len/2' gives a 2x overshoot. This means at max pulse, the "zero point"
      // is far outside the strip.
      uint32_t max_radius = len;
      uint32_t current_radius = (max_radius * effective_val) / 255;

      // Ensure a minimum radius so it doesn't disappear completely (Dry off
      // fix)
      if (current_radius < 2)
        current_radius = 2;
     ...
     * Divides the strip into 2/4/6/8 mirrored segments that dynamically
     * "breathe" in size. Even segments render forward, odd segments render
     * backward. Adds prism glints at the symmetry bounds to enhance the
     * kaleidoscope illusion. Uses a pure sub-pixel triangle wave phase engine
     * to guarantee flawless scrolling mirrors.
     */
    uint16_t mode_kaleidos(void) {
      if (!instance)
        return FRAMETIME;
      ...
      uint32_t cycle_time =
          (ms * (uint32_t)(instance->_segment.speed + 1)) >> 9;
      ...
      uint32_t total_dynamic_phase =
          (uint32_t)(total_base_phase * breath_factor);
      ...
      for (int i = 0; i < len; i++) {
        ...
        // Distance to nearest symmetry bound (0 or 65536 equivalent in
        // fraction)
        uint16_t dist_to_bound =
            (fraction < 32768) ? fraction : (65535 - fraction);
        ...
        if (dist_to_bound < glint_radius) {
          ...
          // Falloff the glint based on exact sub-pixel distance so it doesn't
          // jump
          uint8_t sub_shimmer =
...
#define FM_RESTART 3
    ...
    uint16_t mode_follow_me(void) {
      ...
      FollowMeData *fm =
          reinterpret_cast<FollowMeData *>(instance->_segment.data);
      ...
      const uint32_t *palette =
          getPaletteByIndex(255); // Solid Palette via ID 255
      ...
      // = fast fade. We use scale8 to dim the whole strip, then subtract a
      // constant to kill low-level noise.
      ...
      // 60..89: sub 4. < 60: sub 6. <= 15: sub 8 (Very Aggressive). > 90:
      // sub 2.
      ...
      for (int i = start; i < stop; i++) {
        if (i < light.size()) {
          ...
          // Increased threshold to 20 for absolute clearance of low-level
          // noise.
          if (c.r < 20)
            c.r = 0;
          if (c.g < 20)
      ...
      case FM_MOVING: {
        ...
        for (int j = 0; j < cursor_size; j++) {
          ...
          if (px >= 0 && px < len) {
            ...
            // Uses Solid Palette (forced above), so it's just brightness
            // gradient logic over solid color But palette is forced to solid,
            // so ColorFromPalette returns the solid color (with brightness
            // scaled by index if it was a gradient palette, but here it's
            // solid). Actually, ColorFromPalette with PaletteSolid returns the
            // color regardless of index (usually). Let's rely on standard
            // behavior.
            uint8_t ci = (j * 255) / cursor_size;
...
#define FU_RESTART 4
    ...
    uint16_t mode_follow_us(void) {
      ...
      FollowUsData *fu =
          reinterpret_cast<FollowUsData *>(instance->_segment.data);
      ...
      case FU_PULSE: {
        ...
        for (int k = 0; k < cursor_total && k < len; k++) {
          instance->_segment.setPixelColor(
              k, RGBW32(cfx::scale8(solid_color.r, bri),
                        cfx::scale8(solid_color.g, bri),
                        cfx::scale8(solid_color.b, bri),
                        cfx::scale8(solid_color.w, bri)));
        }
      ...
      case FU_RUN: {
        ...
        for (int i = 0; i < num_parts; i++) {
          ...
          if (i > 0 && fu->parts[i].active && !fu->parts[i - 1].active) {
            // Trigger when this part has moved gap pixels ahead of part i-1's
            // start
          ...
          // Draw ALL parts: active ones at current pos, inactive ones at start
          // pos
          draw_part((int)fu->parts[i].pos, 255);
        }

        // All arrived? -> JOIN delay before strobe
        if (fu->parts[0].arrived && fu->parts[1].arrived &&
            fu->parts[2].arrived) {
          fu->state = FU_JOIN;
          fu->state_start_ms = now;
        }
      ...
      case FU_JOIN: {
        // Brief pause after all parts arrive — hold the reassembled cursor
        // steady
        for (int i = 0; i < num_parts; i++) {
...
#define FLUID_RAIN_NUM_DROPS 5
    uint16_t mode_fluid_rain(void) {
      ...
      uint16_t cycle_len =
          300 - ((intensity * 60) >> 8); // 300 to 240 units total
      ...
      for (int d = 0; d < FLUID_RAIN_NUM_DROPS; d++) {
        ...
        if (c_phase < t_ripple) {
          ...
          drops[d].ripple_rad =
              (time_in_ripple * 15 * 256) / expansion_duration;
      ...
      for (int i = 0; i < len; i++) {
        ...
        for (int d = 0; d < FLUID_RAIN_NUM_DROPS; d++) {
          int dist =
              abs(i_sub - drops[d].center) >> 8; // Integer pixel distance
          if (drops[d].phase == 1) {
            ...
            // While the ripple expands, keep a persistent white dot at the
            // center so the user knows where the ripple originated.
            if (dist == 0) {
              white_add = qadd8(white_add, drops[d].bright);
            } else if (dist == 1) {
            ...
            // Distance from pixel to the exactly ideal ring radius (in
            // sub-pixels)
            int ring_dist_sub = abs(dist_sub - drops[d].ripple_rad);

            // WIDENED RIPPLE: Render a ring 4.0 pixels wide (1024 subpixels)
            // for smooth blending If ring_dist_sub is 0, brightness is 100%. If
    ...
    // Upgraded physics: colliding edges "glue" together (slow down
    // significantly). Visuals: Nodes act as portals/masks for a scrolling
    // palette gradient.
    uint16_t mode_collider(void) {
      if (!instance)
        return FRAMETIME;
      ...
      for (uint16_t n = 0; n < numNodes; n++) {
        ...
        for (int i = r_start; i <= r_stop; i++) {
          ...
          uint8_t w = qadd8(CFX_W(current), pulse_rgbw.w);
          instance->_segment.setPixelColor(idx, RGBW32(r, g, b, w));
        }
      }
