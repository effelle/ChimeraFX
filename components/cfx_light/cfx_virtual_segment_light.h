...
public:
  ...
  void loop() override {
    bool effect_active =
        this->state_ && this->state_->get_effect_name() != "None";
    ...
    if (effect_active || parent_->has_outro()) {
      if (effect_active) {
        // Run the effect immediately, sidestepping the 1s delay and white flash
        auto *eff = this->state_->get_active_effect();
        if (eff) {
          eff->apply();
        }
      }

      // Perform the deferred DMA flush (normally handled by base loop)
      if (this->next_show_) {
